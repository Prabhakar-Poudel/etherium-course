...
describe('Campaigns', () => {
	...
	it('markes caller as the manager', async () => {
		const manager = await campaign.methods.manager().call();
		assert.equal(accounts[0], manager);
	});

	it('allows people to contribute money and mark them contributer', async () => {
	...
	it('allows a manager to make request', async () => {
		...
		assert.equal(accounts[2], request.recepient)
	});
	...
	it('processes request', async () => {
		...
		await campaign.methods
		.createRequest('Foo', web3.utils.toWei('5', 'ether'), accounts[2])
		.send({ from: accounts[0], gas: '1000000' });
